...
def main():
    ...
    while 1:
        ...
        if lastFrame is None:
            ...
            lastFrame = cv2.resize(lastFrame, (int(len(lastFrame)/3), int(len(lastFrame[0])/7)))
            pass

        ret, frame = cap.read() #capture 1 frames
        #same as before...
        frame = cv2.resize(frame, (int(len(frame)/3), int(len(frame[0])/7)))
